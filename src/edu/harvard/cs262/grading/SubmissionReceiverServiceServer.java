...
public class SubmissionReceiverServiceServer implements
	...
	private void lookupStorageService() {
		for (int i = 0; i < NUM_LOOKUP_RETRIES; i++) {
			try {
				Registry registry = LocateRegistry.getRegistry(); // need to specify host
				SubmissionStorageService s = (SubmissionStorageService)
					registry.lookup("SubmissionStorageService");
	...
	public Submission submit(StudentID studentID, AssignmentID assignmentID,
		...
		Submission submission = new SubmissionImpl(studentID, assignmentID, contents);
		try {
	...
	public static void main(String[] args) {
		try {
			...
			SubmissionReceiverService stub = (SubmissionReceiverService) 
